...
class LLMConfig:
    ...
    api_key: str = field(
        default_factory=lambda: os.getenv("GROQ_API_KEY", ""), repr=False
    )
...
class IngestionConfig:
    ...
    documents_dir: str = field(
        default_factory=lambda: str(PROJECT_ROOT / "data" / "documents")
    )
    chroma_dir: str = field(
        default_factory=lambda: str(PROJECT_ROOT / "data" / "chroma_db")
    )
    bm25_path: str = field(
        default_factory=lambda: str(PROJECT_ROOT / "data" / "bm25_index.pkl")
    )
